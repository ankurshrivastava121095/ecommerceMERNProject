                    ...
                    <div className="row">
                        {
                            Array?.isArray(list) && list?.map((val, key) => (
                                <div key={key} className={`col-lg-2 col-md-4 col-sm-4 mb-3 product-card ${val?.productName?.toLowerCase()?.includes(product?.toLowerCase()) || val?.categoryName?.toLowerCase()?.includes(product?.toLowerCase()) ? val?.isDeleted == 0 ? '' : 'd-none' : 'd-none'}`} onMouseEnter={() => setHoveredIndex(key)} onMouseLeave={() => setHoveredIndex(null)}>
